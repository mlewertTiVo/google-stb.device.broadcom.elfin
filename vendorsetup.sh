add_lunch_combo elfin-userdebug
add_lunch_combo elfin-user

# exact same as 'elfin' but without a|b mode support.
add_lunch_combo elfin_nab-userdebug
add_lunch_combo elfin_nab-user

# 'nvi' targets - 'no vendor image' for legacy layout on O+
add_lunch_combo elfin_nvi-userdebug
add_lunch_combo elfin_nvi-user

add_lunch_combo elfin_mini-userdebug
add_lunch_combo elfin_mini-user

add_lunch_combo aosp_elfin-userdebug

add_lunch_combo elfin_hd-userdebug
add_lunch_combo elfin_hd-user

add_lunch_combo elfin_fhd-userdebug
add_lunch_combo elfin_fhd-user

add_lunch_combo elfin_ms10-userdebug
add_lunch_combo elfin_ms10-user

add_lunch_combo elfin_ms11-userdebug
add_lunch_combo elfin_ms11-user

add_lunch_combo elfin_ms12d-userdebug
add_lunch_combo elfin_ms12d-user

add_lunch_combo elfink64-userdebug
add_lunch_combo elfink64-user

add_lunch_combo elfin_msd-userdebug
add_lunch_combo elfin_msd-user

add_lunch_combo elfin_dd-userdebug
add_lunch_combo elfin_dd-user

add_lunch_combo elfin_dd_ms12d-userdebug
add_lunch_combo elfin_dd_ms12d-user

add_lunch_combo elfin_vmx-userdebug
add_lunch_combo elfin_vmx-user
